...
void setRand_bigInt_array(bigInteger x[]){
	...
	for (int i=0;i<NFp;i++)
	{
		len= rand() % 100;     // the length of each coefficient is not larger than 10^100
...
unsigned long long Fp2mul(bigInteger ele1[],bigInteger ele2[],bigInteger res[]){
   ...
   // this part we can print the coefficient of two element which is the form of a+b*x
	...
	bigIntVagueReduction(&v[0], &p);
	
	// v1 = a1*b1
    generMul(&ele1[1],&ele2[1],&v[1]);
	bigIntVagueReduction(&v[1], &p); 	

	// result res0 = v0- v1
	bigIntSubtraction(&v[0],&v[1],&res[0]);
	bigIntVagueReduction(&res[0], &p); 
...
unsigned long long Fp3mul(bigInteger ele1[],bigInteger ele2[],bigInteger res[]){
	...
	*/
	...
	bigIntVagueReduction(&r[2],&p);  // r[2] = (a1+a2)*(b1+b2)
	...
	bigIntVagueReduction(&res[0],&p); // res0
	...
	bigIntVagueReduction(&res[1],&p); // res1
...
unsigned long long Fp4_2input(bigInteger A0[], bigInteger A1[], bigInteger B0[], bigInteger B1[], bigInteger res[]){
    ...
    /* v0 = a0*b0, v1 = a1*b1  t[2] and t[3] receive the noisy clock cycles of Fp2
    ...
    */
	...
	t[4] = Fp2mul(temp1,temp2,temp0); // (A0+A1)*(B0+B1)
...
unsigned long long Fp6_3over2input(bigInteger A0[],bigInteger A1[],bigInteger A2[],bigInteger B0[],bigInteger B1[],bigInteger B2[],bigInteger res[]){
	...
	*/
	...
	bigIntVagueReduction(&res[4],&p);
    bigIntSubtraction(&temp2[1],&v2[1],&res[5]);
	bigIntVagueReduction(&res[5],&p);

    

	return t[1] - t[0] + t[2] + t[3] + t[4] + t[5] + t[6] + t[7];
...
unsigned long long Fp12_2over2over3(bigInteger A00[],bigInteger A01[],bigInteger A10[],bigInteger A11[], bigInteger B00[],bigInteger B01[],bigInteger B10[], bigInteger B11[], bigInteger res[]){
  ...
  */
	...
	t[1] = Fp6_2over3input(A00,A01,B00,B01,V00); 
    t[2] = Fp6_2over3input(A10,A11,B10,B11,V11);
	...
	for(int i=0; i<3;i++){
		...
		bigIntAddition(&B00[i],&B10[i],&temp2[i]); // B00+B10
        bigIntAddition(&B01[i],&B11[i],&temp3[i]); // B01+B11
	} 

	t[3] = Fp6_2over3input(temp0,temp1,temp2,temp3,CZ);// (A0+A1)*(B0+B1)
    ...
    for(int i = 0; i<6; i++){
		...
		bigIntVagueReduction(&res[i+6],&p);
	}

	t[4] = cpucycles();

	return t[4] - t[0] - t[1] - t[2] - t[3];

}


unsigned long long Fp12_2over3over2(bigInteger A00[],bigInteger A01[],bigInteger A02[],bigInteger A10[], bigInteger A11[],bigInteger A12[],bigInteger B00[], bigInteger B01[],bigInteger B02[], bigInteger B10[], bigInteger B11[], bigInteger B12[], bigInteger res[]){
  ...
  */
	...
	t[0] = cpucycles();
	t[1] = Fp6_3over2input(A00,A01,A02,B00,B01,B02,V00);
    t[2] = Fp6_3over2input(A10,A11,A12,B10,B11,B12,V11);
	...
	t[3] = Fp6_3over2input(temp0,temp1,temp2,temp3,temp4,temp5,CZ);// (A0+A1)*(B0+B1)
...
unsigned long long Fp12_3over2over2(bigInteger A00[],bigInteger A01[],bigInteger A10[],bigInteger A11[], bigInteger A20[],bigInteger A21[],bigInteger B00[], bigInteger B01[],bigInteger B10[], bigInteger B11[], bigInteger B20[], bigInteger B21[], bigInteger res[]){
  ...
  */
	...
	t[1] = Fp4_2input(A00,A01,B00,B01,V00);

    t[2] = Fp4_2input(A10,A11,B10,B11,V11);
  
    t[3] = Fp4_2input(A20,A21,B20,B21,V22);
	...
	t[4] = Fp4_2input(temp0,temp1,temp2,temp3,V12);
...
/*---------------------------------------------------------------------------------*/
	...
	t[5] = Fp4_2input(temp0,temp1,temp2,temp3,V01);
    ...
    for(int i = 0; i<4; i++){
		...
		bigIntVagueReduction(&res[i+4],&p);
	}
/*-------------------------------------------------------------------------------------*/
	...
	t[6] = Fp4_2input(temp0,temp1,temp2,temp3,V02);
	...
	for(int i = 0; i<4; i++){
		...
		bigIntVagueReduction(&res[i+8],&p);
	}

	t[7] = cpucycles();

	return t[7]-t[0]-t[1]-t[2]-t[3]-t[4]-t[6];
...
unsigned long long Fp4_direct(bigInteger ele1[],bigInteger ele2[],bigInteger res[]){
   ...
   bigIntVagueReduction(&temp[2],&p);   //temp[2] = (a1+a3)*(b1+b3)-v1-v3+v2
...
//-----------------------------------------------------------------------------------------
   ...
   bigIntSubtraction(&temp[2],&v[3],&temp[2]);
   bigIntVagueReduction(&temp[2],&p);  //temp[2] = (a2+a3)*(b2+b3)-v2-v3
...
//---------------------------------------------------------------------------------------------
   ...
   bigIntSubtraction(&temp[2],&v[3],&temp[2]);
   bigIntVagueReduction(&temp[2],&p);   //temp[2] = (a0+a3)*(b0+b3)-v0-v3
   ...
   bigIntVagueReduction(&res[3],&p);   // (a1+a2)*(b1+b2)-v1-v2
...
unsigned long long Fp12_3overDirect4(bigInteger A0[],bigInteger A1[],bigInteger A2[],bigInteger B0[],bigInteger B1[],bigInteger B2[],bigInteger res[]){
	 ...
	 t[1] = Fp4_direct(A0,B0,V0);
	 t[2] = Fp4_direct(A1,B1,V1);
	 t[3] = Fp4_direct(A2,B2,V2);
	 ...
	 t[4] = Fp4_direct(temp0,temp1,temp2);
	 ...
	 for(int i = 0; i<4; i++){
		 ...
		 bigIntVagueReduction(&res[i],&p);    // THE RESULT OF C0
	 }
...
//------------------------------------------------------------------------------
	 ...
	 //C1 = (A0+A1)*(B0+B1) - V0 -V1 -2V2
     t[5] = Fp4_direct(temp0,temp1,temp2);
     for(int i =0; i<4; i++){
		 ...
		 bigIntVagueReduction(&res[i+4],&p);   // THE RESULT OF C1
	 }
...
//-----------------------------------------------------------------------------------	 
    ...
    //C2 = (A0+A2)*(B0+B2) - V0 + V1 -V2
     t[6] = Fp4_direct(temp0,temp1,temp2);
	 for(int i = 0; i<4; i++){
